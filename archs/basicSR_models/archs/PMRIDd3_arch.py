...
## Implementation from https://github.com/MegEngine/PMRID
import torch
...
class PMRIDd3(nn.Module):

    def __init__(self):
        super().__init__()

        self.conv0 = Conv2D(in_channels=3, out_channels=8, kernel_size=3, padding=1, stride=1, is_seperable=False, has_relu=True)
        #self.enc1 = EncoderStage(in_channels=8, out_channels=32, num_blocks=2)
        self.enc2 = EncoderStage(in_channels=8, out_channels=8, num_blocks=2)
        self.enc3 = EncoderStage(in_channels=8, out_channels=8, num_blocks=2)
        self.enc4 = EncoderStage(in_channels=8, out_channels=8, num_blocks=2)

        self.encdec = Conv2D(in_channels=8, out_channels=8, kernel_size=3, padding=1, stride=1, is_seperable=True, has_relu=True)
        self.dec1 = DecoderStage(in_channels=8, skip_in_channels=8, out_channels=8)
        self.dec2 = DecoderStage(in_channels=8, skip_in_channels=8, out_channels=8)
        self.dec3 = DecoderStage(in_channels=8, skip_in_channels=8, out_channels=8)
        ...
        self.out1 = Conv2D(in_channels=8, out_channels=3, kernel_size=3, stride=1, padding=1, is_seperable=False, has_relu=False)

    def forward(self, inp):

        conv0 = self.conv0(inp)
        #conv1 = self.enc1(conv0)
        conv2 = self.enc2(conv0)
