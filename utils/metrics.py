...
def calculate_nme(gt,pred,device, filter_key=None):
    ...
    left_eye_gt = np.array(gt["iris_coordinates"]["2d"][1]).astype(np.float32) /4 #TODO original images are 1024x1024 not 256x256, so for the moment we divide by 4
    ...
    for face_pred, kps_pred in zip(pred['bbox'],pred['kps']):
        if face_pred[0][-1] is not None and face_pred[0][-1] > curr_confidence:
            ...
            left_mouth_pred = kps_pred[0][6:8]  # Fourth 

            right_mouth_pred = kps_pred[0][8:] #Fifth
...
def calculate_nme_torch(gt,pred,device, filter_key=None):
    """
    Disclaimer: Afaik, there is no canonical NME implementation. 
    The overall formula of the NME between a set of i-indexed ground_truth landmarks {g_i} predictions landmarks {p_i} is
    \sum_{i=0}^{n} l_2(g_i,p_i)/norm_factor
    ...
    left_eye_gt = torch.stack(gt["iris_coordinates"]["2d"][1]) /4 #TODO original images are 1024x1024 not 256x256
    right_eye_gt = torch.stack(gt["iris_coordinates"]["2d"][0]) /4
    left_mouth_gt = torch.stack(gt["landmarks_coordinates"]["2d"][10]) /4 #4 right mouth #10 left mouth #3 point of nose, 1 eye left, 0 eye right
    right_mouth_gt = torch.stack(gt["landmarks_coordinates"]["2d"][4]) /4
    nose_gt = torch.stack(gt["landmarks_coordinates"]["2d"][3]) /4
    interocular_dist = np.linalg.norm(left_eye_gt-right_eye_gt,ord=2)
    norm_factor = interocular_dist*N_LANDMARKS
    curr_confidence = 0
    results = []
    result = torch.tensor(0., device=device)
    for face_pred, kps_pred in zip(pred['bbox'],pred['kps']):
