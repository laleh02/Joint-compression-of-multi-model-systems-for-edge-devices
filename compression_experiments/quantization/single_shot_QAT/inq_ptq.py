...
def main():
    ...
    if 'pretrain_network_g' in experiment_config['path']:
        ...
        if experiment_config['model_name'] == 'YuNET' \
            ...
            if experiment_config['model_name'] == 'indPipelinedYuNET':
                ...
                state_dict = torch.load(experiment_config['enhancers']['detector']['pretrain_network_g'])['state_dict']
                ...
                denoising_enhancer = PMRIDd2()
                state_dict = torch.load(experiment_config['model']['pretrain_network_g'])['state_dict']
                denoiser_state_dict = {".".join(k.split(".")[2:]) : v for k,v in state_dict.items() if 'enhancers.denoiser' in k}
                lle_state_dict = {".".join(k.split(".")[2:]) : v for k,v in state_dict.items() if 'enhancers.light_enhancer' in k}
                lle_enhancer.load_state_dict(lle_state_dict)
                denoising_enhancer.load_state_dict(denoiser_state_dict)
    ...
    if experiment_config['aimet_analyze']:
        ...
        preptq_analyzer.analyze(quant_scheme=QuantScheme.post_training_tf_enhanced,
                            ...
                            default_output_bw=8,
                            config_file=None,
                            results_dir= os.path.join(results_dir, "pre_ptq_results"))
    ...
    ### PTQ - Order of heuristics from https://quic.github.io/aimet-pages/releases/latest/user_guide/adaround.html

    ### Folding BNs
    if experiment_config['ptq']['bnf']:
        start = time.time()
        _ = fold_all_batch_norms(prepared_model, input_shapes=args.input_shape)

        end = time.time()
        print(f"Folding Batch Norms time: {end - start}")
    ### CLE
    if experiment_config['ptq']['cle']:
        start = time.time()
        equalize_model(prepared_model, input_shapes=args.input_shape)
        end = time.time()
        print(f"Layer Equalization time: {end - start}")

    ### Adaround

    ### Adaround needs more samples than Layer MSE Loss estimation
    unlabeled_adaround_dataset = Unlabeled_Dataset(config = experiment_config['datasets']['adaround_dataset'], task = experiment_config['task'])
    adaround_data_loader = create_dataloader(unlabeled_adaround_dataset, experiment_config)
    if experiment_config['ptq']['adaround']:
        start = time.time()
        if experiment_config['task'] == 'Restoration':
            adaround_params = AdaroundParameters(data_loader = adaround_data_loader, num_batches=100,default_num_iterations = 1000)
        else:
            adaround_params = AdaroundParameters(data_loader = adaround_data_loader, num_batches=68,default_num_iterations = 1000)


        prepared_model = Adaround.apply_adaround(prepared_model, dummy_input, adaround_params,
                                    path=results_dir,
                                    filename_prefix="adaround",
                                    default_param_bw=8,
                                    default_quant_scheme=QuantScheme.post_training_tf_enhanced)
        end = time.time()
        print(f"Adaround time: {end - start}")
    ...
    if experiment_config['ptq']['adaround']:
        sim.set_and_freeze_param_encodings(encoding_path=os.path.join(results_dir,"adaround.encodings"))
    ...
    print(f"Compute encodings time: {end - start}")   
    ...
    if experiment_config['task'] == 'Restoration':
       ...
       print("")
        ...
        print(f"EVAL RESULT - Eval results after PTQ : {eval_res}")
        results_dict['after PTQ'] = eval_res
    ...
    if experiment_config['task'] == 'Restoration':
        ...
        quantsim.save_checkpoint(sim, os.path.join(results_dir, "ptq_quantsim_checkpoint"))
    else : 
