...
from archs.PMRIDu3 import PMRIDu3
...
def main():
    ...
    if 'pretrain_network_g' in experiment_config['path']:
        ...
        if experiment_config['model_name'] == 'YuNET':
            ...
            det_img, _ = resize_img(cv2_image, 'AUTO')
            image_metas = [{
    ...
    pruning_ratio = experiment_config.get('pruning_ratio', 0.1)
    iterative_steps = 10
    ...
    if experiment_config['task'] == 'Restoration':
        torch.onnx.export(model, dummy_input,  os.path.join(results_dir, "pruned_model.onnx"), export_params=True)
    else:
        torch.onnx.export(model, dummy_input[0],  os.path.join(results_dir, "pruned_model.onnx"), export_params=True)
